...
earnCoin= (postParams) => {
    return new Promise(function (resolve, reject) {
        ...
        let selectPreviousSteps = "Select * from user_steps where user_id ="+user_id + " order by id desc limit 1";
        connections.ExecuteSelectQuery(selectPreviousSteps)
                        .then(selectPreviousStepsData => { 

                        let previousSetps = 0;
                        let newSteps = steps;
                        if(selectPreviousStepsData.length>0){
                  ...
                  .then(insertStepsData => {
                    ...
                    connections.ExecuteSelectQuery(sqlQuery)
                        .then(data => {
                            ...
                            let checkStep = newSteps/settingCoinStep;
                            let coin = 0;
